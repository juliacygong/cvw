...
module privdec import cvw::*;  #(parameter cvw_t P) (
  ...
  input  logic         VirtModW,                            // current Hypervisor virtual bit
  input  logic         STATUS_TSR, STATUS_TVM, STATUS_TW,   // status bits
  input  logic         HSSTATUS_VTSR, HSTATUS_VTVM, HSTATUS_VTM, // hypervisor status bits
  output logic         IllegalInstrFaultM,                  // Illegal instruction
  output logic         VirtInstrExcepM,                     // Virtual instruction fault
  ...
  logic                VSMode, VUMode;                      // hypervisor virtualization mode
  logic                virtualInstrVU, virtualInstrVS;      // hypervisor virtual instructions
  logic                hlv, hlvx, hsv, hfence;              // hypervisor instructions


  // VU and VS modes
  assign VSMode = VirtModW & (PrivilegeModeW == P.S_MODE);
  assign VUMode = VirtModW & (PrivilegeModeW == P.U_MODE);
  ...
  // Hypervisor Virtual instructions
  // hypervisor virtual machine load and store instructions (VU and VS mode)
  assign hlv = PrivilegedM & (InstrM[31:25] == 7'b0110000 | InstrM[31:25] == 7'b0110010 | InstrM[31:25] == 7'b0110100); // load virtual
  assign hsv = PrivilegedM & (InstrM[31:25] == 7'b0110001 | InstrM[31:25] == 7'b0110011 | InstrM[31:25] == 7'b0110101); // store virtual
  assign hlvx = PrivilegeM & (InstrM[31:20] == 12'b011001000011 | InstrM[31:20] == 12'b011010000011) & rs1zeroM; // load with execute virtualization
  assign hfence = PrivilegedM & (InstrM[31:25] == 7'b0010001 | InstrM[31:25] == 7'b0110001);

  // virtual instruction exceptions in VS and VU mode
  assign virtualInstrVU = VUMode & (hlv | hlvx | hsv | hfence | sretM | sfencevmaM | wfiM);
  assign virtualInstrVS = VSMode & (hlv | hlvx | hsv | hfence |
                                   (sret & HSTATUS_VTSR) | ((sfencevmaM |svinval) & HSTATUS_VTVM) |
                                    (wfiM & HSTATUS_VTM & ~STATUS_TW));

  // virtual instruction fault signal
  assign VirtInstrExcepM = virtualInstrVU | virtualInstrVS;
